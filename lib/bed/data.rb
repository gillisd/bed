Object.send(:remove_const, :Data) if defined?(Data) && !Data.respond_to?(:define)
class ::Data
end

module Bed
  unless defined? Bed::Data
    Bed.const_set(:Data, ::Data)
  end
end
$VERBOSE = false

unless Bed::Data.respond_to?(:define)
  class ::Bed::Data
    def deconstruct
      @__members__.values
    end
      ...
      class << klass
        def new(*values, **named_values)
          if named_values.empty?
            raise ArgumentError, "wrong number of arguments (given #{values.size}, expected 0..#{members.size})" if values.size > members.size

            super(**members.first(values.size).zip(values).to_h)
          else
            raise ArgumentError, "wrong number of arguments (given #{values.size}, expected 0)" unless values.empty?
    ...
    def with(**update)
      ...
      self.class.new(**@__members__, **update)
    end
  end
end
